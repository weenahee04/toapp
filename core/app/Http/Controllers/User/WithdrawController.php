...
{
    protected function makeWithdrawSummary($withdraw, $userData = null)
    {
        $summaryValue = null;

        if ($userData) {
            foreach ((array) $userData as $field) {
    ...
    public function withdrawMoney()
    {
        ...
        $paypal = WithdrawMethod::active()->where('id','=',1)->get();
        $bank = WithdrawMethod::active()->where('id','=',54)->get();
        return view('Template::user.withdraw.methods', compact('pageTitle','withdrawMethod','paypal','bank'));
    }
    ...
    public function withdrawSubmit(Request $request)
    {
        ...
        session()->flash('withdraw_summary', $this->makeWithdrawSummary($withdraw, $userData));
        $notify[] = ['success', 'Withdraw request sent successfully'];
        return to_route('user.withdrawsuccess');
    }

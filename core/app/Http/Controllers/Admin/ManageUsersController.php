...
class ManageUsersController extends Controller
{
    ...
    public function investment($id)
    {
        ...
        $investments  = Investment::searchable(['plan:name'])->where('user_id', $id)->with('plan', 'user')->latest()->paginate(getPaginate());
        $emptyMessage = 'Data Not Found';
        return view('admin.users.investment', compact('pageTitle', 'investments', 'emptyMessage'));
    }
    ...
    {
        $user      = User::findOrFail($id);
        $pageTitle = 'Referrals - ' . $user->fullname . ' (' . $user->username . ')';
        $referrals = User::where('ref_by', $id)->searchable(['username', 'email', 'mobile'])->with('deposits')->paginate(getPaginate());

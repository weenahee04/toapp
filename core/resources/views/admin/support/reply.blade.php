        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-body ">
                    ...
                    <form  id="reply_box" action="{{ route('admin.ticket.reply', $ticket->id) }}" enctype="multipart/form-data" method="post" class="form-horizontal disableSubmission">
                        @csrf
                        ...
                        <div class="row ">
                            ...
                            <div class="col-md-3">
                                <button id="replayTicket" class="btn btn--primary w-100 my-2" type="submit" name="replayTicket" value="1"><i class="la la-fw la-lg la-reply"></i> @lang('Reply')
                                </button>
                            </div>
                        </div>
    ...
    <script>
        ...
        (function($) {
            ...
            $(document).on('click','.delete-reply', function(e) {
              $(document).on('submit','form[action="'+$(this).data('action')+'"]',function(e){
                    e.preventDefault();
                    var formData = new FormData(this);
                    let deleteBtn=$('#delete_confirm');
                    let btnText=$('#delete_confirm').html();
                    ...
                    $.ajax({
                        ...
                        deleteBtn.html(btnText);
                        },
                        success: function(data) {
                            $('#reply_'+data.message_id).remove();
                            notify(data.notify[0], data.notify[1]);
                            $('.modal').modal('hide');
                        },
                        error: function(xhr, status, error) {
                            notify('error','reply deleted failed')
                        }
                    });
              });
            })
            ...
            function newMessage(message){  
                $.ajax({
                url:"{{route('admin.ticket.message')}}",
                method:"post",
